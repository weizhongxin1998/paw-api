<template>
  <n-modal :show="show" preset="card" title="API Docs" style="width: 860px; max-height: 85vh" :mask-closable="false" @update:show="emit('update:show', $event)">
    <template #header-extra>
      <n-button-group size="tiny">
        <n-button :type="viewMode === 'html' ? 'primary' : 'default'" @click="viewMode = 'html'">HTML</n-button>
    ...
    <template #footer>
      <div class="modal-footer">
        ...
        <n-button type="primary" :disabled="!markdownContent" @click="onExportMD">Export MD</n-button>
        <n-button type="primary" :disabled="!htmlContent" @click="onExportHTML">Export HTML</n-button>
      </div>
    </template>
  </n-modal>
</template>
...
import { NModal, NButton, NButtonGroup, NSpin, NResult } from 'naive-ui'
...
async function generateDocs() {
  ...
  try {
    ...
    markdownContent.value = md; htmlContent.value = html
  } catch (err: any) { error.value = err?.message || err?.toString() || 'Unknown error' }
...
function onExportMD() { downloadFile(markdownContent.value, 'api-docs.md', 'text/markdown') }
function onExportHTML() { downloadFile(htmlContent.value, 'api-docs.html', 'text/html') }
</script>

<style scoped>
.docs-loading { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 60px; gap: 12px; color: var(--text-muted); }
