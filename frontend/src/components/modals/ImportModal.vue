    ...
    <template v-else>
      <n-form label-placement="left" label-width="80">
        <n-form-item label="导入格式">
          <n-select v-model:value="format" :options="formatOptions" placeholder="选择导入格式" />
        </n-form-item>
        <n-form-item label="文件路径">
          <n-input-group>
            <n-input v-model:value="filePath" placeholder="选择要导入的文件路径" :disabled="status === 'importing'" />
            <n-button :disabled="status === 'importing'" @click="onBrowse">浏览</n-button>
          </n-input-group>
        </n-form-item>
      </n-form>
      <div class="modal-footer">
        <n-button @click="onClose" :disabled="status === 'importing'">取消</n-button>
        <n-button type="primary" :loading="status === 'importing'" :disabled="!canImport" @click="onImport">导入</n-button>
      </div>
    </template>
  </n-modal>
</template>
...
async function onImport() {
  ...
  try {
    ...
    resultType.value = 'success'; resultTitle.value = '导入成功'
    resultDesc.value = `成功导入 ${result.collections} 个集合, ${result.requests} 个请求`
    status.value = 'result'; emit('imported')
  } catch (err: any) {
    resultType.value = 'error'; resultTitle.value = '导入失败'
